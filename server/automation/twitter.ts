...
async function takeScreenshot(page: any, mediaDir: string, stepName: string) {
  try {
    ...
    console.log(
      `[Automation] Debug screenshot saved to ${screenshotPath}. Accessible at /media/debug_twitter_${stepName}.png`,
    );
  } catch (err: any) {
    console.error(
      `[Automation] Failed to take debug screenshot for ${stepName}:`,
      err.message,
    );
  }
}
...
export async function publishToTwitter(
  ...
  try {
    ...
    console.warn(
      `[Automation] X/Twitter navigation warning/timeout, checking if DOM is ready anyway:`,
  ...
  if (
    ...
    throw new Error(
      "Authentication failed: X/Twitter redirected to a login or signup page. Please refresh your session cookies.",
    );
  }
  ...
  if (localMediaPaths.length > 0) {
    ...
    const fileInputSelector =
      'input[type="file"][data-testid="fileInput"], input[type="file"]';
    try {
      await page.waitForSelector(fileInputSelector, { timeout: 10000 });
      const fileInput = await page.$(fileInputSelector);
      if (fileInput) {
        console.log(
          `[Automation] Uploading ${localMediaPaths.length} media files to Twitter...`,
        );
      ...
      console.error(
        `[Automation] Twitter media upload selector/action failed:`,
        err,
      );
    }
  }
  ...
  const buttonSelector =
    '[data-testid="tweetButton"], [data-testid="tweetButtonInline"], div[role="button"][data-testid="tweetButtonInline"]';
  await page.waitForSelector(buttonSelector, { timeout: 10000 });

  if (localMediaPaths.length > 0) {
    console.log(
      `[Automation] Media uploaded. Waiting for X/Twitter upload processing to complete (checking button disabled state)...`,
    );
    // Wait for the button to be enabled (aria-disabled !== "true").
    // We poll every 2 seconds for up to 90 seconds.
    let isReady = false;
    for (let attempt = 1; attempt <= 45; attempt++) {
      const isDisabled = await page.evaluate((sel: any) => {
      ...
      if (!isDisabled) {
        console.log(
          `[Automation] X/Twitter publish button is now active (aria-disabled != true)! Upload/processing complete.`,
        );
        isReady = true;
        break;
      }
      ...
      console.log(
        `[Automation] Publish button is disabled (uploading/processing)... attempt ${attempt}/45`,
      );
      ...
      if (attempt % 5 === 0) {
        await takeScreenshot(
          page,
          mediaDir,
          `upload_progress_attempt_${attempt}`,
        );
      }
    }
    if (!isReady) {
      console.warn(
        `[Automation] Warning: X/Twitter publish button remained disabled after 90 seconds. Proceeding to click anyway.`,
      );
    }
  }
